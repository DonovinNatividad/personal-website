...
const Project: React.FC<ProjectProps> = ({ title, description, technologies, image, githubLink }) => {
    return (
        <a href={githubLink} target="_blank" rel="noopener noreferrer" className=' lg:w-1/3 p-4 transform hover:scale-105 transition-transform duration-200'>
            <div className='flex flex-col items-start bg-gray-900 rounded-xl p-4 h-full border-b border-black'>
                <Image src={image} alt={title} className='w-full object-cover rounded-xl' height={256} width={384} />
